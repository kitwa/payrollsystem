...
@Component({
  ...
  template: `<main class="auth-page"><section class="auth-panel"><div class="auth-panel__inner"><p class="auth-eyebrow">Account recovery</p><h1>Reset password</h1><p class="auth-muted">Choose a new password for your Payroll SA account.</p>@if (message()) { <div class="alert alert-success">{{ message() }} <a routerLink="/login">Sign in</a></div> } @if (error()) { <div class="alert alert-danger">{{ error() }}</div> }<form [formGroup]="form" (ngSubmit)="submit()"><label class="form-label" for="reset-password">New password</label><div class="input-group mb-3"><input id="reset-password" class="form-control form-control-lg" [type]="showPassword() ? 'text' : 'password'" formControlName="password" autocomplete="new-password"><button class="btn btn-outline-secondary" type="button" (click)="showPassword.set(!showPassword())"><i class="bi" [class.bi-eye]="!showPassword()" [class.bi-eye-slash]="showPassword()"></i></button></div><label class="form-label" for="reset-confirm">Confirm password</label><div class="input-group mb-3"><input id="reset-confirm" class="form-control form-control-lg" [type]="showConfirm() ? 'text' : 'password'" formControlName="confirm" autocomplete="new-password"><button class="btn btn-outline-secondary" type="button" (click)="showConfirm.set(!showConfirm())"><i class="bi" [class.bi-eye]="!showConfirm()" [class.bi-eye-slash]="showConfirm()"></i></button></div><button class="btn btn-primary btn-lg w-100" type="submit" [disabled]="form.invalid || loading()">{{ loading() ? 'Resetting...' : 'Reset password' }}</button></form></div></section></main>`
})
